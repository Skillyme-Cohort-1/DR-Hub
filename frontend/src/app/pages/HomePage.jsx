...
import {
  ...
  ArrowRight, Star, ChevronRight, ArrowUpRight,
} from "lucide-react";
...
function formatReviewSnippet(quote, limit = REVIEW_SNIPPET_LIMIT) {
  ...
  if (cleanQuote.length <= limit) return cleanQuote;
...
export default function HomePage() {
  ...
  const extendedTestimonials = useMemo(() => {
    if (testimonials.length > 1) return [...testimonials, testimonials[0]];
    return testimonials;
  }, [testimonials]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted && data?.reviews?.length) setTestimonials(data.reviews.slice(0, 3));
      } catch (err) {
  ...
  useEffect(() => {
    if (extendedTestimonials.length <= 1) return undefined;
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main id="main-content">
        ...
        <section className="relative flex min-h-screen items-center justify-center overflow-hidden pt-17">
          ...
          />
          <div className="absolute inset-0 bg-linear-to-b from-black/75 via-black/55 to-background" />
        ...
        {/* Testimonials  */}
        <section id="testimonials" className="scroll-mt-24 border-t border-border bg-muted/30 px-4 py-16 sm:px-6 lg:py-20">
          <div className="mx-auto max-w-7xl">
            <div className="mb-10 flex flex-col items-center justify-between gap-4 sm:flex-row sm:items-start">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 rounded-full border border-[#E87722]/40 bg-[#E87722]/8 px-4 py-2 text-sm font-medium text-[#E87722] transition-all duration-200 hover:border-[#E87722]/70 hover:bg-[#E87722]/15"
              >
                Google Reviews
            ...
            <p className="mx-auto mb-8 max-w-2xl text-center text-base text-foreground/55 sm:mx-0 sm:text-left">
            ...
            <div className="relative h-48 overflow-hidden">
              ...
              >
                {extendedTestimonials.map((testimonial, index) => (
                  ...
                  >
                    <div className="h-full rounded-2xl border border-border bg-card p-6 shadow-lg">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="ml-auto flex items-center gap-1 text-yellow-400">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              className={`h-4 w-4 ${i < (testimonial.rating || 5) ? 'fill-current' : 'text-foreground/20'}`}
                            />
                          ))}
                        </div>
                      </div>
                      <blockquote className="mt-4 text-lg italic leading-relaxed text-foreground/75">
